...
theorem eventually_timeoutEndpointRawProfile_le_shellError
  ...
  have hOldLower : Ahi + H * R * Q * E ≤
    ...
    exact add_le_add_left
      (mul_le_mul_of_nonneg_left (le_add_of_nonneg_left hEdy0) hHQ0) _
  have hCompare : Ahi + H * R * Q * Etimeout ≤
      2 * (Ahi + H * R * Q * (Edy + E)) := by
    calc
      Ahi + H * R * Q * Etimeout ≤
          Ahi + H * R * Q * (2 * E) := add_le_add_left hlowMul _
  ...
  calc
    ...
    _ ≤ 2 * (movingEndpointProfileConstant P C D *
      ...
      simpa [Ahi, H, R, Q, E, Edy, L, b, x,
        timeoutTimeSupportConstant, movingTimeSupportConstant] using hScalar2
