...
def reverseProduct (n h : ℕ) : ℚ :=
  ∏ i in Finset.range h, reverseStepFactor n i
...
theorem reverseProduct_eq_prod_loss (n h : ℕ) :
    reverseProduct n h =
      ∏ i in Finset.range h, (1 - reverseLoss n i) := by
...
theorem reverseProduct_bounds {Y n h : ℕ}
  ...
  have hsum :
      ∑ i in Finset.range h, reverseLoss n i ≤
        (h : ℚ) / (2 * (Y : ℚ)) := by
    calc
      ∑ i in Finset.range h, reverseLoss n i ≤
          ∑ _i in Finset.range h, (1 / (2 * (Y : ℚ)) : ℚ) := by
  ...
  · calc
      1 - (h : ℚ) / (2 * (Y : ℚ)) ≤
          1 - ∑ i in Finset.range h, reverseLoss n i := by linarith
      _ ≤ ∏ i in Finset.range h, (1 - reverseLoss n i) :=
