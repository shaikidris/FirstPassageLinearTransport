...
-/
import FirstPassageLinearTransport.MovingEndpointProfile
...
theorem moving_entropy_term_le_buffer
  ...
  have hK : 0 < K := by
    dsimp [K]
    positivity
  have hLr : 0 < (L : ℝ) := by exact_mod_cast (show 0 < L by simpa [L] using hL)
...
/-- The canonical literal moving shell family satisfies the exact error
profile consumed by the already formal natural-density assembly. -/
