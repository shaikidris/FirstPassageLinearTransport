...
/-!
# Historical fixed two-regime parameter package

The public timeout/shrinking route uses the endpoint scalars from
`FixedPolylogParameters`, but not this fixed two-regime package.  Keeping the
package here prevents its declarations from entering the canonical `Main`
environment while preserving the earlier route as an independently buildable
library.
-/
...
/-- Scalar endpoint selection for the retained fixed two-regime route. -/
...
/-- The fixed endpoint selector leaves a strict retained entropy margin. -/
...
/-- Complete scalar package consumed only by the retained two-regime route. -/
...
/-- Every admissible triple has one retained fixed two-regime package. -/
theorem exists_fixedPolylogParameterPackage
  ...
  have hHiRate0 : 0 < adjustableEntropyRate lambdaHi tHi :=
    adjustableEntropyRate_pos (by dsimp [lambdaHi]; norm_num) htHi0
