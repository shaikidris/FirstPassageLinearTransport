...
theorem card_biUnion_le_sum {α β : Type*} [DecidableEq α] [DecidableEq β]
  ...
  | @insert a s ha ih =>
      ...
      calc
        ...
        _ ≤ (f a).card + ∑ x in s, (f x).card := by omega
        _ = ∑ x in insert a s, (f x).card := by simp [ha]
...
theorem arbitraryTarget_linear_transport {M Y H : ℕ} (B : Finset ℕ)
  ...
  calc
    ...
    _ ≤ ∑ _t in Finset.range H, ∑ _y in B, K := by
      apply Finset.sum_le_sum
      intro t ht
      apply Finset.sum_le_sum
      intro y _hy
