...
The manuscript's V3.1 moving-endpoint refinement is now a public Lean
headline.  The internal modules formalize the uniform `M^(-1/2)` binomial
prefactor in manuscript (3.3a), the moving low-rank parameters in (6.11), the
real-to-lattice rounding, the literal low-rank landing-density producer with
endpoint rate minus `O(1/L)`, the moving feasible-time support in Lemma 6.4,
the moving first-bad transport/profile adapter, the exact-rate sharp
`q^(-1/2)` low-rank profile, quantitative startup with `M0 <= L`, exact
critical-buffer scalar closure, and literal same-witness execution.  The final
assembly bounds the actual no-witness shell set and exposes Theorem 1.1 in
`Main.lean`.  The fixed-polylogarithmic theorem remains as Corollary 1.2(1).
...
| Lemma 6.4 / Prop 6.5 adapter, moving first-bad transport | `FirstPassageLinearTransport.movingFirstBadSourcesAtRank_subset_transport`; `movingFirstBadSourcesAtRank_card_le`; `movingSeparatedFailureEnvelope_density_terminalProfile` | `MovingFirstBad.lean`; `MovingProfile.lean` |
| Proposition 6.5, sharp `q^(-1/2)` low profile at the unchanged endpoint rate | `FirstPassageLinearTransport.exists_exact_sharp_critical_low_series_bound`; `moving_low_firstBad_sharp_exact_sum_le` | `MovingSharpTail.lean`; `MovingSharpProfile.lean` |
