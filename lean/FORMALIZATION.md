...
The access-controlled referee snapshot is
...
`9837ca9d65d9de6fad1ef4381750ca688774e608`.  From this directory, run:

```text
lake build FirstPassageLinearTransport.PaperDependencyAudit FirstPassageLinearTransport.PaperAudit FirstPassageLinearTransport.Main
```
...
real-to-lattice rounding, and the literal low-rank landing-density producer
with endpoint rate minus `O(1/L)`.  The remaining formal gap is the moving
low-phase transport/profile adapter and its connection to the public
assembly.  The public fixed-polylogarithmic theorem still matches Corollary
1.2(1).  This boundary prevents the stronger moving theorem from being
attributed to `Main.lean` before that final adapter is complete.
...
|---|---|---|
| Theorem 1.1, moving endpoint | **analytic producer formalized; no public Lean declaration yet** | `SharpEntropyBarrier.lean`; `MovingLowParameters.lean`; `MovingLowDensity.lean`; moving profile/assembly connection pending |
