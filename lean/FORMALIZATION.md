...
The frozen public fixed-parameter referee snapshot is
...
`9837ca9d65d9de6fad1ef4381750ca688774e608`.  The current private branch adds
two complete formal realizations of the moving-endpoint theorem.  The V3.2
primary route uses the manuscript's low-rank timeout event; the retained V3.1
alternate uses an all-prefix moving low certificate.  Both have public
`Main.lean` declarations with the same literal conclusion.
From this directory, run:

```text
lake build
lake build FirstPassageLinearTransport.MovingEndpointAudit FirstPassageLinearTransport.TimeoutEndpointAudit FirstPassageLinearTransport.PaperDependencyAudit FirstPassageLinearTransport.PaperAudit FirstPassageLinearTransport.Main
```

The first command is the fail-loudly package gate and must remain: the
targeted audit command alone does not compile orphaned or newly added modules.
...
all-prefix moving-route report is generated by
`FirstPassageLinearTransport/MovingEndpointAudit.lean`.
...
no-witness shell set and exposes Theorem 1.1 in `Main.lean`.
...
cross-check rather than a manuscript dependency.  The two routes have
...
Two quantitative endpoint ranges are likewise paper-only at present.  The
fixed-`A` public theorem returns a positive logarithmic exceptional exponent
but does not expose every `gamma < kappa_* (A - A_FP)` in its type.  The
stretched-log public theorem formalizes every strict
`sigma < 1 - delta`; it does not expose the manuscript's endpoint power
`sigma = 1 - delta`.
...
|---|---|---|
| Theorem 1.1, timeout moving endpoint | `FirstPassageLinearTransport.QuantitativeCollatzMain.collatz_first_passage_timeout_moving_polylogarithmic_natural_density_descent` | `TimeoutEndpointNaturalDensity.lean`; `Main.lean` |
...
| Formal alternate, all-prefix moving endpoint | `FirstPassageLinearTransport.QuantitativeCollatzMain.collatz_first_passage_moving_polylogarithmic_natural_density_descent` | `MovingLowSetup.lean`; `MovingEndpointParameters.lean`; `MovingEndpointProfile.lean`; `MovingEndpointAsymptotics.lean`; `MovingEndpointWitness.lean`; `MovingEndpointNaturalDensity.lean`; `Main.lean` |
...
| Formal alternate, all-prefix same-witness landing/clock/ceiling | `FirstPassageLinearTransport.eventually_movingEndpointGood_has_shellWitness` | `MovingExecution.lean`; `MovingOrbitCeiling.lean`; `MovingEndpointWitness.lean` |
...
| Theorem 1.3, landing and orbit ceiling; every strict exceptional power below the endpoint | `FirstPassageLinearTransport.QuantitativeCollatzMain.collatz_first_passage_quantitative_stretched_exceptional_count`; `collatz_first_passage_stretched_log_descent_with_orbit_ceiling` | `QuantitativeNaturalDensityDescent.lean`; `OrbitCeiling.lean`; `Main.lean` |
...
timeout chain and literal assembly. `MovingEndpointAudit.lean` audits the
retained all-prefix alternate.  The public, timeout, alternate-route,
dependency, and full-package builds are the required release gates.
