...
the complete formal moving-endpoint chain and its public `Main.lean` theorem.
From this directory, run:

```text
lake build
lake build FirstPassageLinearTransport.MovingEndpointAudit FirstPassageLinearTransport.PaperDependencyAudit FirstPassageLinearTransport.PaperAudit FirstPassageLinearTransport.Main
```
...
the internal moving-producer axiom report is generated by
`FirstPassageLinearTransport/MovingEndpointAudit.lean`.
The repository is private and can be shared with referees on request.

The moving-endpoint theorem is a public Lean headline.  The internal modules
formalize one complete proof through an all-prefix moving low producer: its
...
The V3.2 manuscript proves the same theorem statement by a shorter low-rank
timeout argument in Lemma 6.3 and Proposition 6.4.  That substitution removes
the moving low all-prefix certificate from the written dependency chain.  The
timeout proof route is not yet separately formalized: Lean kernel-checks the
public theorem surface through the independent all-prefix route, not a second
proof term mirroring the new manuscript derivation.  This distinction is
load-bearing and is also stated in the manuscript disclosure.
...
|---|---|---|
| Theorem 1.1, moving endpoint | `FirstPassageLinearTransport.QuantitativeCollatzMain.collatz_first_passage_moving_polylogarithmic_natural_density_descent` | `MovingLowSetup.lean`; `MovingEndpointParameters.lean`; `MovingEndpointProfile.lean`; `MovingEndpointAsymptotics.lean`; `MovingEndpointWitness.lean`; `MovingEndpointNaturalDensity.lean`; `Main.lean` |
| Theorem 1.1, quantitative startup | `FirstPassageLinearTransport.eventually_movingLowStageSetup_M0_le` | `MovingLowSetup.lean` |
| Theorem 1.1, literal sharp shell profile | `FirstPassageLinearTransport.exists_eventually_movingEndpointGood_shellError` | `MovingEndpointProfile.lean`; `MovingEndpointAsymptotics.lean` |
| Theorem 1.1, same-witness landing/clock/ceiling | `FirstPassageLinearTransport.eventually_movingEndpointGood_has_shellWitness` | `MovingExecution.lean`; `MovingOrbitCeiling.lean`; `MovingEndpointWitness.lean` |
| Lemma 6.3, timeout target and combined feasible-time support | **paper proof; timeout route not separately formalized** | manuscript Section 6 |
| Proposition 6.4, timeout first-bad profile and same-witness conclusion | **paper proof; public conclusion formalized through the independent all-prefix route** | manuscript Section 6; public `Main.lean` theorem |
...
| Corollary 1.4, raw `10.44 log n` clock | `FirstPassageLinearTransport.QuantitativeCollatzMain.collatz_first_passage_raw_stretched_log_natural_density_descent` | `RawNaturalDensityDescent.lean`; `Main.lean` |
...
`MovingEndpointAudit.lean` runs the same trust check on the complete internal
moving chain and literal assembly.  The public, moving-producer, cut-vertex,
