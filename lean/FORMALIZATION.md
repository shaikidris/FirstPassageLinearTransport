...
the formal moving-endpoint producer, time-support theorem, direct first-bad
transport, and conditional terminal-profile socket without promoting a public
moving headline.  From this directory, run:
...
The manuscript's V3.1 moving-endpoint refinement is not yet a public Lean
headline.  The internal modules now formalize the uniform `M^(-1/2)` binomial
...
the moving first-bad transport/profile adapter, and the exact-rate sharp
`q^(-1/2)` low-rank profile (`MovingFirstBad.lean`, `MovingProfile.lean`,
`MovingSharpTail.lean`, `MovingSharpProfile.lean`).  The remaining formal
gaps are uniform moving-stage startup, the eventual `Delta_M` shell-density
and same-witness producer, and its connection to the public assembly.  The public
fixed-polylogarithmic theorem still matches Corollary 1.2(1).  This boundary
prevents the stronger moving theorem from being attributed to `Main.lean`
before that final wiring is complete.
...
|---|---|---|
| Theorem 1.1, moving endpoint | **producer, time support, first-bad transport, and exact-rate sharp low profile formalized; no public Lean declaration yet** | `SharpEntropyBarrier.lean`; `MovingLowParameters.lean`; `MovingLowDensity.lean`; `MovingTimeSupport.lean`; `MovingFirstBad.lean`; `MovingProfile.lean`; `MovingSharpTail.lean`; `MovingSharpProfile.lean`; uniform startup, eventual shell producer, and public assembly pending |
...
| Lemma 6.4, moving low-stage direct passage, decreasing potential, and square-root feasible-time support | `FirstPassageLinearTransport.MovingRecertificationRun.directFirstPassage`; `MovingRecertificationRun.certified_endpoint_shell_eq`; `MovingRecertificationRun.deviation_add_potential_le`; `eventually_movingFeasibleTimes_card_lt_sqrt`; `eventually_nonempty_movingLowStageSetup` | `MovingTimeSupport.lean` |
...
`MovingEndpointAudit.lean` runs the same trust check on the internal moving
producer, moving time-support theorem, and conditional assembly.  The public,
moving-producer, cut-vertex, dependency, and full-package builds passed on
2026-08-11.
