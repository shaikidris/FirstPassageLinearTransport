...
| E02 | DEFERRED-TO-FREEZE | Lean disclosure lacks one immutable accessible artifact pointer and full metadata. | At freeze, add commit/tag or archive URL, Lean version, Mathlib revision, build command, and audit paths. | A referee can fetch and replay exactly the claimed artifact. |
